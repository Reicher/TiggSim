...
TS.GameEnd.prototype = {
  ...
  create: function(){
    ...
    var back = this.add.sprite(50, 50, 'infoBox');
    ...
    if(this.playerWellbeing <= 0 || !this.goalReached){
      endText = this.add.text(300, 80, 'Ingen lycka denna gång!',{ font: "40px Arial"});
      endText.anchor.set(0.5);
      endExplaination = this.add.text(70, 130, 'Du forsätter på samma sätt i \nmånader och lyckas knappt försöja\ndig själv. Det är dags att åka hem\nför den här gången.', { font: "30px Marlett"} );
    }
    else{
      endText = this.add.text(300, 80, 'En lyckad resa!',{ font: "40px Arial"});
      endText.anchor.set(0.5);
      endExplaination = this.add.text(70, 130, 'Du fortsätter i samma stil och har\nefter en tid lyckats samlat ihop \nlite pengar.', { font: "30px Arial"} );
    }

    var restartText = this.add.text(300, 320, '(Tryck för att försöka igen)', { font: "20px Arial" });
